...
class PatchTST(nn.Module):
    ...
    def __init__(
        ...
        self.n_layers = config.n_layers if config.n_layers else 3
        self.d_model = config.d_model if config.d_model else 128
        self.n_heads = config.n_heads if config.n_heads else 16
        self.shared_embedding = config.shared_embedding if config.shared_embedding else True
        self.d_ff: int = config.d_ff if config.d_ff else 256
        self.norm: str = config.norm if config.norm else "BatchNorm"
        self.attn_dropout: float = config.attn_dropout if config.attn_dropout else 0.0
        self.dropout: float = config.dropout if config.dropout else 0.0
        self.act = config.act if config.act else "gelu"
        self.res_attention: bool = config.res_attention if config.res_attention else True
        self.pre_norm: bool = config.pre_norm if config.pre_norm else False
        self.store_attn: bool = config.store_attn if config.store_attn else False
        self.pe = config.pe if config.pe else "zeros"
        self.learn_pe: bool = config.learn_pe if config.learn_pe else True
        self.head_dropout = config.head_dropout if config.head_dropout else 0
        self.head_type = config.head_type if config.head_type else "prediction"
        self.individual = config.individual if config.individual else False
        self.y_range: Optional[tuple] = config.y_range if config.y_range else None
